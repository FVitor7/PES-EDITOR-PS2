...
public class WENPanel extends JPanel implements ActionListener {
	...
	public void refresh() {
		int wen = (of.toInt(of.data[54]) << 16) + (of.toInt(of.data[53]) << 8)
				+ of.toInt(of.data[52]);
	...
	public void setWEN(int newWen) {
		if (newWen >= 0 && newWen <= 99999) {
			of.data[52] = of.toByte(newWen & 0xFF); //offset
			of.data[53] = of.toByte((newWen & 0xFF00) >>> 8); //offset
			of.data[54] = of.toByte((newWen & 0xFF0000) >>> 16); //offset
			//of.data[5208] = of.toByte(newWen & 0xFF);
			of.data[5216] = of.toByte(newWen & 0xFF); //offset
			//of.data[5209] = of.toByte((newWen & 0xFF00) >>> 8);
			of.data[5217] = of.toByte((newWen & 0xFF00) >>> 8); //offset
			//of.data[5210] = of.toByte((newWen & 0xFF0000) >>> 16);
			of.data[5218] = of.toByte((newWen & 0xFF0000) >>> 16); //offset
			refresh();
		} else {
			field.setText("");
