...
public class OptionFilter extends FileFilter {
	...
	public boolean accept(File f) {
		 ...
		 * else if (fileIsPCOption(f)) { return true; }
		 */
		return false;
	}
	...
	/*
	 * private boolean fileIsPCOption(File f) { boolean result = false; try { if
	 * (f.canRead() && f.length() == OptionFile.LENGTH) { byte[] id = new
	 * byte[4]; RandomAccessFile rf = new RandomAccessFile(f, "r"); rf.read(id);
	 * rf.close(); if (id[0] == -97 && id[1] == 114 && id[2] == -31 && id[3] ==
	 * -58) { result = true; } } } catch (IOException e) { result = false; }
	 * return result; }
	 */

	public String getDescription() {
		return "PES 2014 PS2 Option File"; //version
	}
}
