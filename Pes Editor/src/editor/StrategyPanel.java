...
public class StrategyPanel extends JPanel {
	...
	public StrategyPanel(OptionFile opf, SquadList l, PositionList pl) {
		...
		String[] items = { "No Strategy", "Centre Att.", "R. Side Att.",
		 ...
		 * Formations.setStratAuto(of, squad, auto); refresh(squad); } } });
		 */
	...
	public void refresh(int s) {
		...
		/*
		 * if (Formations.getStratAuto(of, squad)) { auto = true;
		...
		boolean ol = false;
		for (int i = 0; i < 4; i++) {
			int strat = Formations.getStrat(of, squad, i);// of.data[670608 + i
			// + (628 * squad) +
			// 6232];
