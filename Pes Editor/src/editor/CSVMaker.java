...
public class CSVMaker {
	...
	public boolean makeFile(OptionFile opf, File dest, boolean headings,
		...
		try {
			...
			if (create) {
				for (int player = Player.firstEdit; player < 32952; player++) { //offset
					writePlayer(out, player);
				}
			}
			out.close();
	...
	private void writeHeadings() throws IOException {
		String[] head = { "GK  0", "SW  2", "CB  3", "SB  4", "DMF  5",
				...
				"SHOT POWER", "SHOT TECHNIQUE", "FREE KICK ACCURACY", "SWERVE",
				"HEADING", "JUMP", "TECHNIQUE", "AGGRESSION", "MENTALITY",
				"GOAL KEEPING", "TEAM WORK", "CONSISTENCY",
	...
	private void writeClassicStatus(int player) throws IOException {
		...
		int b2;
		int cNat = 0;
		if (nat == 6 || nat == 8 || nat == 9 || nat == 14 || nat == 15
				|| nat == 44 || nat == 45) {
			if (nat == 6) {
				cNat = 60; //squadnumber
			}
			...
			for (int ip = 0; ip < 23; ip++) {
				...
				b2 = of.toInt(of.data[Squads.slot23 + 1 + (46 * cNat)
						+ (ip * 2)]);
				if (((b2 << 8) | b1) == player) {
	...
	private void writePlayer(RandomAccessFile out, int player)
		...
		for (int i = 0; i < Stats.abilitySpecial.length; i++) {
			out
					.writeBytes(Stats.getString(of, player,
							Stats.abilitySpecial[i]));
			out.write(seperator);
		}
