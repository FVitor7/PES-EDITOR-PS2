...
public class EmblemPanel extends JPanel implements MouseListener {
	...
	public EmblemPanel(OptionFile opt, EmblemImportDialog fid, TeamPanel tp) {
		...
		for (int l = 0; l < Emblems.totalB6; l++) {
			...
			flagButton[l].addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent b) {
					...
					if (flagImpDia.of2Open || flagImpDia.of2v6Open || flagImpDia.of2v5Open) {
						options = options1;
					} else {
						options = options2;
					}
					...
					if ((flagImpDia.of2Open || flagImpDia.of2v6Open || flagImpDia.of2v5Open) && n == 3) {
						int replacement = -1;
						if (is128) {
							...
							if (replacement != -1) {
								replacement = replacement - Emblems.total6;
		...
		add2Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent t) {
				...
				if (emblem != -1) {
					if (emblem > Emblems.total6 - 1) {
						emblem = emblem - Emblems.total6;
	...
	public void refresh() {
		...
		for (int i = 0; i < Emblems.count128(of); i++) {
			flagButton[Emblems.totalB6 - 1 - i].setIcon(new ImageIcon(Emblems
					.get128(of, i, !trans, true)));
			flagButton[Emblems.totalB6 - 1 - i].setVisible(true);
		}

		for (int i = Emblems.count16(of); i < Emblems.totalB6
