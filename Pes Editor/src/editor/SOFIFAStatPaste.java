...
public class SOFIFAStatPaste extends JDialog 
{
	...
	public static String[] positionNames2 = 
		...
		for (int i=0; i<lines.length; i++)	
		{
			try
			{
					...
					if (strIn(f,"Nationality"))
					{
						...
						for (int k=0; k<nationalities.length; k++)
							if (nation.toUpperCase().equals(nationalities[k].toUpperCase()))
								{ index = k; break;}
						String normalNation = Stats.nation6[index];
				...
				else
				{
					...
					f = f.replace("★","").replace("*","").trim();
					for (int z=0; z<positionNames2.length; z++)
						if (strIn(positionNames2[z],f) && strIn(f,positionNames2[z]))
						{
							debugWrite("Found special "+positionNames2[z]);
							retForm.posPanel.checkBox[z].setSelected(true);	
							break;
						}
				}
				debugWrite("");
	...
	public SOFIFAStatPaste(Frame owner, PlayerDialog ret) 
	{
